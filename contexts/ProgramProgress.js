...
export async function ensureActiveProgram(programName, startWeek = 1, startDay = 1) {
  ...
  if (!s.active || (programName && s.active.programName !== programName)) {
    s.active = {
      programName: programName || s.active?.programName || 'Get Yolked 4.0',
      pointer: { week: startWeek, day: startDay },
      lastCompleted: null
    };
    await setState(s);
  }
...
export async function markCompleteAndAdvance({ programName, week, day }) {
  const s = await getState();
  if (!s.active) {
    s.active = { programName: programName || 'Get Yolked 4.0', pointer: { week, day }, lastCompleted: null };
  }
  if (programName && s.active.programName !== programName) {
    s.active = { programName, pointer: { week, day }, lastCompleted: null };
  }
...
export async function getNextUp() {
  ...
  const items = (preset.exercises || []).map(e => ({
    name: e.name,
    sets: e.sets ?? 3,
    reps: e.reps ?? 10
  }));
