...
if(!/name\s*=\s*['"]History['"]/.test(s)){fs.writeFileSync(p,s);console.log('no History route found');process.exit(0)}
s=s.replace(/(<\s*(?:Tab|Stack)\.Screen\b[^>]*name\s*=\s*['"]History['"][\s\S]*?component=)\{?([A-Za-z0-9_$.]+)\}?/m,'$1{HistoryFallback}');
fs.writeFileSync(p,s);console.log('wired History to HistoryFallback');
